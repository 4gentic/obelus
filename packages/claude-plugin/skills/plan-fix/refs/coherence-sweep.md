...
If fewer than two substantive blocks exist, skip the sweep — it is vacuous with one or zero edits. Emit `coherence: 0` and move on. This is NOT a performance shortcut — at N ≥ 2 the sweep always runs. (The desktop's prelude reports `coherence-sweep: skipped (substantive blocks < 2)` when this skip applies; trust that signal.)
