...
**Why.** Reviewing a PDF is stateful in a way that spans several components — the PDF pane, the margin gutter, the review pane, the category picker. A shared store means each surface reads and writes the same truth, with persistence handled by the repository it is wired to.
