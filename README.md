...
**Desktop app** — Tauri v2 build. Adds source editing in CodeMirror, a git-style diff review pane, project folders, managed Typst and Tectonic *(Beta)* engines, and Claude Code or OpenCode in-app. Downloads land under [Releases](https://github.com/4gentic/obelus/releases) once the v1 pipeline ships.
...
Under OpenCode, copy the plugin folder manually — see [`packages/claude-plugin/README.md`](packages/claude-plugin/README.md) for the exact paths. Skills: `/apply-revision`, `/apply-fix`, `/write-review`, `/fix-compile`.
