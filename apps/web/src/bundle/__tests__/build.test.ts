import { Bundle } from "@obelus/bundle-schema";
...
describe.skipIf(!hasIdb)("buildBundle", () => {
  ...
  it("produces a bundle that round-trips through Bundle.parse", async () => {
    ...
    const round = Bundle.parse(JSON.parse(JSON.stringify(bundle)));
    expect(round.annotations).toHaveLength(1);
    expect(round.papers[0]?.pdf?.relPath).toBe("paper.pdf");
    expect(round.papers[0]?.title).toBe("Test paper");
    expect(round.annotations[0]?.anchor.kind).toBe("pdf");
  });
...
function plainBundle(overrides: { note?: string; quote?: string } = {}): BundleType {
  return {
    bundleVersion: "1.0",
    ...
    papers: [
      {
        ...
        pdf: { relPath: "paper.pdf", sha256: "a".repeat(64), pageCount: 12 },
