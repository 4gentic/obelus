...
- **Use `pdfjs-dist` v5 directly.** No viewer wrapper. Worker loaded via Vite `?worker` import — never URL-constructor.
- **Text layer via the canonical `TextLayer` class from `pdfjs-dist`** (fed by `streamTextContent()`). Do not reimplement span positioning. `getBoundingClientRect` is forbidden for stored-highlight rect math; it appears in `SelectionListener.tsx` only for live mouse-gesture geometry (see the file header), where no anchor exists yet.
