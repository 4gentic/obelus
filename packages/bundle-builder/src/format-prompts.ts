...
export function formatFixPrompt(input: PromptInput): string {
  const header = [
    ...
    "",
    "> In Claude Code with the Obelus plugin installed, run `/obelus:apply-review <bundle-path>` on the JSON bundle instead of following this Markdown.",
    "",
...
export function formatReviewPrompt(input: PromptInput): string {
  ...
  const lines: string[] = [
    ...
    `Source PDF: \`${input.paper.pdfFilename}\` (sha256 \`${input.paper.pdfSha256}\`)`,
    "",
    "> In Claude Code with the Obelus plugin installed, run `/obelus:draft-review <bundle-path>` on the JSON bundle instead of following this Markdown.",
    "",
    'Generate a journal-style review of this paper based on the reviewer\'s marks below. **Do not** edit any source file. **Do not** issue a verdict (no "accept", "reject", "revise"). **Do not** invent annotations or citations. Voice: observational, declarative, slightly archaic. No exclamations.',
    "",
