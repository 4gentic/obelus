...
pub async fn resolve_claude_path() -> Option<PathBuf> {
    // The env override is a developer convenience for testing against a
    // specific claude build; in release builds we ignore it so a poisoned
    // env on an end-user's machine can't redirect `claude_spawn` to an
    // arbitrary binary.
