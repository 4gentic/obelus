...
// Reject any statement whose first keyword isn't on this list. Defense-in-depth
// against a renderer-compromise scenario: the CSP already blocks untrusted JS,
// but if it ever slipped through, this closes the DROP/ATTACH/PRAGMA paths.
const ALLOWED_PREFIXES: &[&str] = &["INSERT", "UPDATE", "DELETE", "SELECT"];
...
pub async fn db_tx_batch(
    ...
    for stmt in stmts {
        let kw = first_keyword(&stmt.sql).to_uppercase();
        if !ALLOWED_PREFIXES.iter().any(|p| *p == kw) {
            return Err(format!("db_tx_batch: statement prefix `{kw}` is not allowed"));
        }
